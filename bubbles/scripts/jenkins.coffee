...
jenkinsBuild = (msg, buildWithEmptyParameters, prequalJob) ->
    url = process.env.HUBOT_JENKINS_URL
    job = querystring.escape msg.match[1]
    params = msg.match[3]
    command = if buildWithEmptyParameters then "buildWithParameters" else "build"
    path = if params then "#{url}/job/#{job}/buildWithParameters?#{params}" else "#{url}/job/#{job}/#{command}"
    path = if prequalJob then "#{url}/view/Pre-Qual%20Services/job/#{job}/#{command}" else path
    ...
    req.post() (err, res, body) ->
        if err
          ...
          if prequalJob
            msg.reply "(#{res.statusCode}) Build started for #{job} #{url}/view/Pre-Qual%20Services/job/#{job}"
          else
